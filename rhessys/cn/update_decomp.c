                  ...
                  struct patch_object    *patch)
{
    ...
    double init_SOM_C = get_total_SOM_C(cs_soil);
    double removedC = 0.0;
    ...
    if (fpi < base_rate) {
        ...
        cdf->litr2c_hr += min(extra_plitr2c_loss,max(0.,cs_litr->litr2c - cdf->litr2c_hr));
        cdf->litr3c_hr += min(extra_plitr3c_loss,max(0.,cs_litr->litr3c - cdf->litr3c_hr));
        cdf->litr4c_hr += min(extra_plitr4c_loss,max(0.,cs_litr->litr4c - cdf->litr4c_hr));
    }
    ...
    cs_litr->litr3c       -= cdf->litr3c_hr; //10012025LML
    ...
    if (fabs(balance) > 1.e-12) {//09222023LML ZERO)
        printf("\n Decomp N doesn't balance by %lf ", balance);
    }

    //Checking C balance
    removedC += cdf->litr1c_hr +
                cdf->litr2c_hr +
                cdf->litr3c_hr +
                cdf->litr4c_hr +
                cdf->soil1c_hr +
    ...
    double end_SOM_C = get_total_SOM_C(cs_soil);

    double C_balance = (end_SOM_C - init_SOM_C) +
                       (end_litterC - init_litterC) +
                       removedC;
    if (fabs(C_balance) > 1.e-12) {
