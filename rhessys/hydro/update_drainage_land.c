                     ...
                     int verbose_flag)
{
    ...
    /*--------------------------------------------------------------*/
    //LML note:
    //1. The surface pool is updated
    //2. The soil leaching and surface outflow go to downstream neighbors or streamflow
    //3. The soil IS NOT updated
    ...
    //10022025LML check N (virtual) balance
    double init_total_N = patch[0].soil_ns.nitrate +
    ...
    double init_soil_N = patch[0].soil_ns.nitrate +
                         patch[0].soil_ns.sminn +
                         patch[0].soil_ns.DON;
    double end_total_N = 0;
    double end_soil_N = 0;
    double fluxout_soil_N = 0;
    double fluxout_N = 0;
    ...
    if (command_line[0].grow_flag > 0) {
        ...
        patch[0].soil_ns.NO3_Qout += leached[LNO3];                             //LML note: These Qout will be used in compute_subsurface_routing to update soil N pools
        ...
        fluxout_N += leached[LNO3] + leached[LNH4] + leached[LDON];
    }
    patch[0].Qout += pQout;                                                     //Will be counted to update water table in later routine
    //09132022LML note: seems won't be triggered other than run only one patch.
    if (patch[0].innundation_list[0].num_neighbours == 0 || patch[0].drainage_type == STREAM) {
    ...
    if (command_line[0].grow_flag > 0) {
            ...
            end_total_N += lpools[LNO3] + lpools[LNH4] + lpools[LDON]
                           - leached[LNO3] - leached[LNH4] - leached[LDON];     //virtual since soil N not updated yet
    }
    ...
    end_soil_N = end_total_N; //virtual
    end_total_N += patch[0].surface_DON + patch[0].surface_NO3 + patch[0].surface_NH4; //surface N has been updated
    ...
    if (command_line[0].noredist_flag == 0) {
      ...
      if (numnb == 0 || patch[0].drainage_type == STREAM) {
          //All leached goes to stream
          patch[0].streamflow_DOC += DOC_leached_to_patch / patch[0].area;
          patch[0].streamflow_DON += DON_leached_to_patch / patch[0].area;
          patch[0].streamflow_NH4 += NH4_leached_to_patch / patch[0].area;
          patch[0].streamflow_NO3 += NO3_leached_to_patch / patch[0].area;
      } else {
          double export_N = NO3_leached_to_patch +
          ...
          for (int j = 0; j < numnb; j++) {
            ...
            import_N += fgamma * DON_leached_to_patch * neigh[0].area +
                        fgamma * NH4_leached_to_patch * neigh[0].area +
                        fgamma * NO3_leached_to_patch * neigh[0].area;
    ...
    //#pragma omp parallel for
      ...
      if (numnb == 0 || patch[0].drainage_type == STREAM) {
          //All leached goes to stream
          patch[0].streamflow_DOC += DOC_leached_to_surface / patch[0].area;
          patch[0].streamflow_DON += DON_leached_to_surface / patch[0].area;
          patch[0].streamflow_NH4 += NH4_leached_to_surface / patch[0].area;
          patch[0].streamflow_NO3 += NO3_leached_to_surface / patch[0].area;
      } else {
          for (int j = 0; j < numnb; j++) {
            struct  neighbour_object *nbo = &patch[0].surface_innundation_list[d].neighbours[j];
            struct  patch_object *neigh = nbo->patch;
      ...
      double patch_N_balance = end_total_N - init_total_N + fluxout_N;  //total N: soil + surface
      fluxout_soil_N = patch[0].soil_ns.NO3_Qout +
