...
//            double, double, double, double, double, double, double,double *);
    ...
    for (k = 0; k < n_timesteps; k++) {
        ...
        for (int i = 0; i < hillslope->route_list->num_patches; i++) {
            ...
            double init_soil_N = patch[0].soil_ns.nitrate +
                                 patch[0].soil_ns.sminn +
                                 patch[0].soil_ns.DON;
            double soil_flux_N = 0;
            ...
            if (grow_flag > 0) {
                ...
                soil_flux_N += (patch[0].soil_ns.NO3_Qin - patch[0].soil_ns.NO3_Qout) +
                        (patch[0].soil_ns.NH4_Qin - patch[0].soil_ns.NH4_Qout) +
                        (patch[0].soil_ns.DON_Qin - patch[0].soil_ns.DON_Qout);
            }
    ...
    /*-----------------------------------------------------------------------*/
            ...
            double end_soil_N = patch[0].soil_ns.nitrate +
                                 patch[0].soil_ns.sminn +
                                 patch[0].soil_ns.DON;
            double soil_N_balance = end_soil_N - init_soil_N - soil_flux_N;
            if (fabs(soil_N_balance) > 1.e-12) {
                printf("ERROR:MASS BALANCE %f @%d\n",soil_N_balance,__LINE__);
            }
        } /* end i */
        ...
        if (k == (n_timesteps - 1)) //end day
        {
            ...
            for (int i = 0; i < hillslope->route_list->num_patches; i++) {
              ...
              double init_soil_N = patch[0].soil_ns.nitrate +
                                   patch[0].soil_ns.sminn +
                                   patch[0].soil_ns.DON;
              double soil_flux_N = 0;  //out is negtive; in is positive
              double init_surf_N = patch[0].surface_DON +
                                   patch[0].surface_NH4 +
                                   patch[0].surface_NO3;
              double surf_flux_N = 0;
              ...
              if ((patch[0].sat_deficit
                ...
                if (grow_flag > 0) {
                    ...
                    soil_flux_N -= leached[LDON] + leached[LNO3] + leached[LNH4];
                    surf_flux_N += leached[LDON] + leached[LNO3] + leached[LNH4];
                }
            }
            ...
            if (((excess = patch[0].detention_store
                ...
                double fexcess = excess / patch[0].detention_store;
                double excess_NO3 = fexcess * patch[0].surface_NO3;
                double excess_NH4 = fexcess * patch[0].surface_NH4;
                double excess_DON = fexcess * patch[0].surface_DON;
                double excess_DOC = fexcess * patch[0].surface_DOC;
                surf_flux_N -= excess_DON + excess_NO3 + excess_NH4;

                if (patch[0].drainage_type == STREAM) {
                    if (grow_flag > 0) {
                        patch[0].streamflow_DON += excess_DON;
                        patch[0].streamflow_DOC += excess_DOC;

                        patch[0].streamflow_NO3 += excess_NO3;
                        patch[0].streamNO3_from_surface += excess_NO3;
                        patch[0].hourly[0].streamflow_NO3 += excess_NO3;
                        patch[0].hourly[0].streamflow_NO3_from_surface += excess_NO3;

                        patch[0].streamflow_NH4 += excess_NH4;

                        patch[0].surface_DON -= excess_DON;
                        patch[0].surface_DOC -= excess_DOC;
                        patch[0].surface_NO3 -= excess_NO3;
                        patch[0].surface_NH4 -= excess_NH4;
                    }
                    ...
                    if (grow_flag > 0) {
                        ...
                        patch[0].surface_NO3 -= excess_NO3;
                        patch[0].surface_NH4 -= excess_NH4;
                        patch[0].surface_ns_leach += excess_NO3;
                    }
                    patch[0].detention_store -= excess;
                    patch[0].Qout_total += excess;
#ifdef LIU_CHECK_WATER_BALANCE
            ...
            if ((grow_flag > 0) && (infiltration > ZERO)) {
                double finfil = infiltration / patch[0].detention_store;

                patch[0].soil_ns.DON += (finfil * patch[0].surface_DON);
                patch[0].soil_cs.DOC += (finfil * patch[0].surface_DOC);
                patch[0].soil_ns.nitrate += (finfil * patch[0].surface_NO3);
                patch[0].soil_ns.sminn += (finfil * patch[0].surface_NH4);

                soil_flux_N += finfil * (patch[0].surface_DON +
                                         patch[0].surface_NO3 +
                                         patch[0].surface_NH4);
                surf_flux_N -= finfil * (patch[0].surface_DON +
                                         patch[0].surface_NO3 +
                                         patch[0].surface_NH4);

                patch[0].surface_NO3 -= (finfil * patch[0].surface_NO3);
                patch[0].surface_NH4 -= (finfil * patch[0].surface_NH4);
                patch[0].surface_DOC -= (finfil * patch[0].surface_DOC);
                patch[0].surface_DON -= (finfil * patch[0].surface_DON);
            ...
            {
                if (patch[0].drainage_type == STREAM || patch[0].innundation_list[0].num_neighbours == 0) { //10022025LML Need check!
                    hillslope[0].hillslope_return_flow += (patch[0].return_flow) * patch[0].area;
                    ...
                    hillslope[0].hillslope_detention_store += patch[0].detention_store * patch[0].area;
                }
            }
            ...
            /* the accumulator is updated in update_hillslope_patch_accumulator.c in hillslope_daily_F.c*/
                ...
                double end_soil_N = patch[0].soil_ns.nitrate +
                                     ...
                                     patch[0].soil_ns.DON;
                double soil_N_balance = end_soil_N - init_soil_N - soil_flux_N;
                double end_surf_N = patch[0].surface_DON +
                                    patch[0].surface_NH4 +
                                    patch[0].surface_NO3;
                double surf_N_balance = end_surf_N - init_surf_N - surf_flux_N;

                if (fabs(soil_N_balance) > 1.e-12 || fabs(surf_N_balance) > 1.e-12) {
