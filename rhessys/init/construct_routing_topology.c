...

{
    ...
    int assign_neighbours_in_hillslope (struct neighbour_object *,
        int, struct hillslope_object *,  FILE *);
    ...
    for (i=0; i< num_patches; ++i) {
        ...
        /*--------------------------------------------------------------*/
          ...
          //  innundation_list->num_neighbours = num_neighbours;
          //}
          if ((num_neighbours == 0) && (patch[0].drainage_type != STREAM)) {
              //10022025LML set patch is STREAM if none downstream patches
              printf("\n Set patch %d as STREAM.\n", patch[0].ID);
              patch[0].drainage_type = STREAM;
          }
          innundation_list->num_neighbours = num_neighbours;
